...
public class Functions {
    ...
    /**
     * 
     * @param sEmail Email van de gebruiker die wil inloggen.
     * @param sWachtwoord Wachtwoord van de gebruiker die wil inloggen.
     * @return nummer van het recht van de gebruiker. 0 betekent geen account geveonden. 1 is brandweer en 2 is Overheid of Gemeente.
     */
    // Inlog functie
    ...
    /**
     * 
     * @param sNaam Naam van de nieuwe gebruiker.
     * @param sWerk Werk van de nieuwe gebruiker.
     * @param sRegio Regio van de nieuwe gebruiker. Alleen ingevuld als werk gelijk is aan brandweer.
     * @param sEmail Email van de nieuwe gebruiker
     * @param sWachtwoord Wachtwoord van de nieuwe gebruiker.
     * @param sRecht Recht van de nieuwe gebruiker.
     * @return Recht teruggeven van de gebruiker.
     */
    // Registreer functie
    ...
    /**
     * 
     * @param sEmail Email van de gebruiker.
    ...
    /**
     * 
     * @param sRegio Regio van de brandweer.
     * @return Gegevens van de nieuwste telling van de kazerne.
     */
            ...
            if (!rsset.isBeforeFirst()) {
                ...
                try {
                    ...
                    for (int i = 1; i < root.length(); ++i) {
                        ...
                        // Kijken of er resultaat is in de array
                        if(arr != null && arr.length() > 0 ){
                            ...
                            for (int json = 0; json < arr.length(); json++) {
                                ...
                                // Geen resultaat gevonden met hetzelfde jaar
                                else{
                                    // Jaar ophalen en er een jaar van af halen
                                    int inewJaar = iJaar - 1;
                                    
                                    // Query maken
                                    // Deze query zoekt of er lokaal een kazerne is die gelijk is aan regio, jaar. 
                                    // Hier haalt hij de nieuwste op, alleen het eerste resultaat wordt opgehaald
                                    String sQueryNew = "SELECT * FROM kazerne WHERE regio = '" + sRegio + "' AND jaar = " + inewJaar + " ORDER BY kid DESC LIMIT 1";
                                    //System.out.println(sQuery);
                                    
                                    // Statement maken
                                    stm = conn.createStatement();
                                    // Query mkaen
                                    stm.executeQuery(sQueryNew);
                                    // Resultaten ophalen
                                    rsset = stm.getResultSet();
                                    
                                    // Check of er resultaat is
                                    if (!rsset.isBeforeFirst()) {
                                        if (sRegion.matches(sRegio) && iYear == inewJaar) {
                                            // Variabelen aanmaken voor de kolommen die in de array komen
                                            int iKazernID = arr.getJSONObject(json).getInt("ID");
                                            int iProfessioneelMan = arr.getJSONObject(json).getInt("Male_4");
                                            int iProfessioneelVrouw = arr.getJSONObject(json).getInt("Female_5");
                                    ...
                                    // In de database is wel resultaat gevonden met het nieuwe jaar
                                    else{
    ...
    /**
     * 
     * @return Alle nieuwste tellingen van de kazernes.
     */
    ...
    public ObservableList<EntKazerne> getOverviewKazernes() {
        ...
        try {
            ...
            for (int i = 1; i < root.length(); ++i) {
                ...
                // Resultaten van het jaar daarvoor ophalen
                else{
                    // Jaar ophalen en er een jaar afhalen
                    int iVorigJaar = iJaar - 1;
                    
                    // Variabele voor JSON url aanmaken
                    URI newURI = new URI("https://opendata.cbs.nl/ODataApi/OData/71482eng/TypedDataSet?$filter=Years%20eq%20%27" + iVorigJaar + "JJ00%27");
                    // JSON bestand openen
                    JSONTokener newTokener = new JSONTokener(newURI.toURL().openStream());
                    // Variabele aanmaken voor het object in JSON
                    JSONObject newRoot = new JSONObject(newTokener);
                    
                    // Door de JSON objecten heel loopen
                    for (int newI = 1; newI < newRoot.length(); ++newI) {
                        // JSON Array ophalen in het object
                        JSONArray newArray = newRoot.getJSONArray("value");
                    
                        // Door de Array heen loopen van JSON
                        for (int json = 0; json < newArray.length(); json++) {
                            // Regio en jaar ophalen 
                            String sRegioJSON = newArray.getJSONObject(json).getString("Region");
                            String iYear = newArray.getJSONObject(json).getString("Years");
                            ...
                            if (!rsset.isBeforeFirst()) {
                                ...
                                // Query maken
                                // Deze query haalt eerst het laatste id omdat dat het meest resent is en dan groupt hij op regio om 1 resultaat te krijgen
                                String sQueryVorigJaar = "SELECT MAX(kid) AS kid, regio, jaar, professioneel_man, professioneel_vrouw, vrijwilliger_man, vrijwilliger_vrouw, "
                                        + "niet_operationeel_hbo_wo, niet_operationeel_mbo, niet_operationeel_lbo, tekort\n"
                                        + "FROM kazerne \n"
                                        + "WHERE jaar = " + iVorigJaar + " AND regio = '" + sRegioJSON + "' \n"
                                        + "GROUP BY regio";

                                // Statement maken
                                stm = conn.createStatement();
                                // Query mkaen
                                stm.executeQuery(sQueryVorigJaar);
                                // Resultaten ophalen
                                rsset = stm.getResultSet();
                                
                                // Check of er resultaat is van het jaar daarvoor
                                if (!rsset.isBeforeFirst()) {
                                    // Variabele aanmaken van de kolommen
                                    int iKazernID = newArray.getJSONObject(json).getInt("ID");
                                    int iProfessioneelMan = newArray.getJSONObject(json).getInt("Male_4");
                                    int iProfessioneelVrouw = newArray.getJSONObject(json).getInt("Female_5");
                                    Long LongProfessioneelHogeMiddenRank = newArray.getJSONObject(json).optLong("HigherAndMidLevelRanks_6", 0);
                                    Long LongProfessioneelLageRank = newArray.getJSONObject(json).optLong("LowerLevelRanks_9", 0);
                                    int iVrijwilligerMan = newArray.getJSONObject(json).getInt("Male_11");
                                    int iVrijwilligerVrouw = newArray.getJSONObject(json).getInt("Female_12");
                                    Long LongVrijwilligerHogeMiddenRank = newArray.getJSONObject(json).optLong("HigherAndMidLevelRanks_13", 0);
                                    Long LongVrijwilligerLageRank = newArray.getJSONObject(json).optLong("LowerLevelRanks_16", 0);
                                    int iOpleidingHboWo = newArray.getJSONObject(json).getInt("HboWoLevel_18");
                                    int iOpleidingMbo = newArray.getJSONObject(json).getInt("MboLevel_19");
                                    int iOpleidingLbo = newArray.getJSONObject(json).getInt("LboLevel_20");
                                    String sTekort = "";
                                    ...
                                    ovKazernes.add(new EntKazerne(iKazernID, sRegioJSON, iJaarJSON, iProfessioneelMan, iProfessioneelVrouw, iProfessioneelHogeMiddenRank, iProfessioneelLageRank, iVrijwilligerMan, iVrijwilligerVrouw, iVrijwilligerHogeMiddenRank, iVrijwilligerLageRank, iOpleidingHboWo, iOpleidingMbo, iOpleidingLbo, sTekort));
                                } else{
                                    // Resultaat gevonden van vorig jaar
                                    // Door Database resultaten heen lopen
                                    while (rsset.next()) {
                                        // Variabele aanmaken voor de database regio en jaar
                                        String sRegioDB = rsset.getString("regio");
                                        ...
                                        // Regio en jaar komen overeen met resultaat database
                                            ...
                                            ovKazernes.add(new EntKazerne(iKazernID, sRegioDB, iJaarDB, iProfessioneelMan, iProfessioneelVrouw, iProfessioneelHogeMiddenRank, iProfessioneelLageRank, iVrijwilligerMan, iVrijwilligerVrouw, iVrijwilligerHogeMiddenRank, iVrijwilligerLageRank, iOpleidingHboWo, iOpleidingMbo, iOpleidingLbo, sTekort));
                                        }
                                    }
                                }
                            // Resultaat van dit jaar
                            } else {
                                // Door Database resultaten heen lopen
                                while (rsset.next()) {
                                    // Variabele aanmaken voor de database regio en jaar
                                    ...
                                    // Als regio en jaar overeen komen met de database dan het database resultaat gebruiken
                                    if (sRegioDB.matches(sRegioJSON) && iJaarDB == iJaar) {
                                    // Regio en jaar komen overeen met resultaat database
                                        // Variabelen aanmaken voor de kolommen die in de array komen
                                        // De ranken krijgen een 0 omdat ze niet opgevraagd worden maar wel in de contructor zitten
                                        int iKazernID = rsset.getInt("kid");
    ...
    /**
     * 
     * @param sRegio Van de kazerne.
     * @return Lijst met alle tellingen van een kazerne.
     */
    ...
    /**
     * 
     * @param sRegio Regio van de kazerne.
     * @param iJaar Jaar van de telling.
     * @param iProfessioneelMan Nieuw aantal Professionele mannen.
     * @param iProfessioneelVrouw Nieuw aantal Professionele vrouwen.
     * @param iProfessioneelHogerMiddenRank Nieuw aantal Professionele leden met een hoge of midden rank.
     * @param iProfessioneelLageRank Nieuw aantal Professionele leden met een lage rank.
     * @param iVrijwilligMan Nieuw aantal vrijwillige mannen.
     * @param iVrijwilligVrouw Nieuw aantal vrijwillige vrouwen.
     * @param iVrijwilligHogeMiddenRank Nieuw aantal vrijwilligers met een hoge of midden rank.
     * @param iVrijwilligLageRank Nieuw aantal vrijwilligers met een lage rank.
     * @param iOpleidingHboWo Nieuw aantal niet operationele leden die een HBO of WO opleidingsniveau hebben.
     * @param iOpleidingMbo Nieuw aantal niet operationele leden die een MBO opleidingsniveau hebben.
     * @param iOpleidingLbo Nieuw aantal niet operationele leden die een LBO opleidingsniveau hebben.
     * @param sTekort Tekort ja of nee.
     * @return getal om te checken of het gelukt is. 
     */
